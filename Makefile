...
.PHONY: setup
setup: ## Install dependencies (need sudo and make already installed)
	sudo apt-get update
...
.PHONY: codecov
codecov: ## Run tests with code coverage and generate XML report for Codecov
	$(POETRY) run pytest --cov=src --cov-branch --cov-report=xml

.PHONY: build
build: ## Build the library (source and wheel files)
	$(POETRY) build
